...
func jsonImageEditInputs(request dto.ImageRequest) ([]imageInputRef, imageInputRef, error) {
	...
	if len(bytes.TrimSpace(rawImages)) == 0 {
		rawImages = request.Extra["images"]
	}
	if len(rawImages) > 0 {
		var items []struct {
		...
		if err := common.Unmarshal(rawImages, &items); err != nil {
	...
	mask := imageInputRef{}
	rawMask := request.Mask
	if len(bytes.TrimSpace(rawMask)) == 0 {
		rawMask = request.Extra["mask"]
	}
	if len(rawMask) > 0 {
		var maskObj struct {
			ImageURL string `json:"image_url"`
		...
		if err := common.Unmarshal(rawMask, &maskObj); err != nil {
			return nil, imageInputRef{}, err
		}
		mask = imageInputRef{
