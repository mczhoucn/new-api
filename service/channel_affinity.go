...
func channelAffinityRecoveryClearPrefixes(rules []operation_setting.ChannelAffinityRule, channel *model.Channel) ([]string, bool) {
	...
	for _, rule := range rules {
		if len(matchingChannelAffinityModels(rule, models)) == 0 {
			continue
		}
		...
		if rule.IncludeModelName {
			// The write path keys affinity by request model. Recovery only has the
			// channel's configured model list, so clear at rule scope.
			if prefix := buildChannelAffinityPrefix(rule, "", ""); prefix != "" {
		...
		for _, group := range groupParts {
			if prefix := buildChannelAffinityPrefix(rule, "", group); prefix != "" {
				prefixSet[prefix] = struct{}{}
			}
		}
	}
